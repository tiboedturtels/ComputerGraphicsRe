...
void CirclePoints(int x, int y, int centerx, int centery)
{
...
void EllipsePoints(int x, int y, int centerx, int centery)
{
	putPixel(centerx + x, centery + y);
	putPixel(centerx + x, centery - y);
	putPixel(centerx - x, centery + y);
...
void drawEllipse(int x0, int y0, int Rx, int Ry)
{
	float x1 = 0;
	float y1 = Ry;
	float D = 1 / pow(Rx, 2) + 1 / (4 * pow(Ry, 2));

	int consecutiveDecreases = 0;

	EllipsePoints(x1, y1, x0, y0);
	while (Ry * x1 / (sqrt(1 - pow(x1 / Rx, 2)) * pow(Rx, 2)) >= -1)
	{
		if (D < 0)
		{
			D += (2 * x1 + 3) / pow(Rx, 2);
		...
		{
			D += (2 * x1 + 3) / pow(Rx, 2) + (-2 * y1 + 2) / pow(Ry, 2);
			y1--;
	...
	float x2 = Rx;
	float y2 = 0;
	D = 1 / pow(Ry, 2) + 1 / (4 * pow(Rx, 2));

	consecutiveDecreases = 0;

	EllipsePoints(x2, y2, x0, y0);
	while (Rx * y2 / (sqrt(1 - pow(y2 / Ry, 2)) * pow(Ry, 2)) >= -1)
	{
		...
		{
			D += (2 * y2 + 3) / pow(Ry, 2) + (-2 * x2 + 2) / pow(Rx, 2);
			x2--;
			consecutiveDecreases++;
		}
		y2++;
